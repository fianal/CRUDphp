...
if (isset($_POST['update'])) {
    ...
    if (empty($nama_makanan) || strlen($nama_makanan) == 0) {
        echo "<script>window.alert('Field is required'); window.location='?page=makanan'</script>";
    } elseif (!preg_match("/^[a-zA-Z ]*$/", $nama_makanan)) {
        echo "<script>window.alert('Only letters and white space allowed'); window.location='?page=makanan'</script>";
